// Question 10:
//  Adding Comments: Choose two of the programs you’ve
//  written, and add at least one comment to each. If the programs 
//  are straightforward at this point, just add your name and the 
//  current date at the top of each program file. Then, write one 
//  sentence describing what the program does.

// Saad baseer khan, 28-apr-24

// this code prints personal messsage 
let personName="Saad baseer khan"
console.log(`Hellow ${personName} would you like to play cricket with us ? `);
